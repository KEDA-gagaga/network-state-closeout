...
请先阅读仓库中的 INSTALL_WITH_CODEX.zh-CN.md，按照其中的 Agent 流程完成安装；安装后开启新任务，再引导我初始化私人网络状态。
```

仓库已经提供 Codex 可识别的插件目录。完整设计理由、安装命令、初始化顺序和安全边界见 [INSTALL_WITH_CODEX.zh-CN.md](INSTALL_WITH_CODEX.zh-CN.md)。
...
推荐直接让 Codex 按 [INSTALL_WITH_CODEX.zh-CN.md](INSTALL_WITH_CODEX.zh-CN.md) 操作。命令行安装入口为：
