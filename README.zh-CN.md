...
还在每次打开新任务、切换项目或更换设备时，都要向 Agent 逐条解释你的主机、服务、代理和访问路径？

还在不同笔记与聊天记录之间翻找，重新拼凑每台设备的 Tailscale IP、Nginx 反代、Docker 端口、域名、TLS 和连接关系？

这些问题的本质并不是“还记得不够多”，而是持续变化的网络知识没有一份统一、可信、容易维护的来源。把所有细节直接固化进一个庞大的 skill 同样不是理想答案：网络状态会变化，真实端点属于私人信息，过期和无关内容还会占用 Agent 的上下文。
...
`network-state-closeout` 借鉴了这套思想，并把它应用到需要长期维护的私人网络状态：稳定的分类规则、写入原则、快速校验、安全边界和同步门禁留在公开插件中；持续变化的设备、服务、地址、访问路径与排错经验保存在用户自己的全局私人 skill 中，并可通过 GitHub 私人仓库跨设备共享。Codex 每次只读取当前任务需要的分类卡片，并自主维护已经确认的稳定事实。

从 Tailscale 设备地址，到 Nginx 反代、域名和 TLS，再到 Docker 容器及其端口、SSH、VPN 与跨设备访问路径，这些零散信息都会被整理成可持续维护、按需读取的网络卡片。

日常确认的事实直接写入网络卡片。只有一项已完成的操作具有长期收尾价值时，才分别判断是否需要状态卡或过程卡；过程卡还必须有可靠的真实过程证据。

最终得到的不是一份很快过期的静态清单，而是一套可以随着环境变化持续维护的私人知识工作流：状态留在自己的全局私人 skill 中，可以只在本机维护；能力通过插件复用；只有启用 GitHub 私人同步时，多台设备上的 Agent 才共享同一份网络资料。每次仍只把相关部分读进上下文。
...
仓库已经提供 Codex 可识别的插件目录。完整设计理由、安装命令、初始化顺序和安全边界见英文 Agent 指令 [INSTALL_WITH_CODEX.md](INSTALL_WITH_CODEX.md)。
...
推荐直接让 Codex 按英文 Agent 指令 [INSTALL_WITH_CODEX.md](INSTALL_WITH_CODEX.md) 操作。命令行安装入口为：
