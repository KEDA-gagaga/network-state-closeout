...
维护个人网络真正困难的，往往不是记住某条命令，而是让不同设备上的 Codex 始终拿到同一份、足够新、可以验证的事实：哪台设备提供什么服务，访问路径从哪里到哪里，哪些状态已经确认，问题再次出现时应该从哪里开始排查。

把所有细节直接写进一个 skill 并不是理想答案。网络状态会变化，真实端点又属于私人信息；资料一旦和插件代码绑在一起，既不方便分享，也容易让过期内容长期占据上下文。

Codex 内置的 OpenAI Docs skill 提供了一个很直接的参考：它没有把持续变化的官方手册固化在 skill 里，而是把来源选择、时效校验、失败边界和检索路线保留为稳定工作流；真正回答问题前，再从可信来源刷新当前版本，生成可定位的目录，并读取与问题有关的章节。它解决的核心问题不是“保存更多知识”，而是始终保留一条通往最新知识的可靠路径。

OpenAI Docs 面向的是只读的官方知识；`network-state-closeout` 则把同样的分层方法延伸到需要长期维护的私人网络状态：
...
> [!IMPORTANT]
> **安全边界：只同步网络事实，绝不保存密钥。** 密码、token、API key、私钥、cookie、恢复码、一次性验证码和任何含凭据的 URL，都禁止进入私人状态、项目卡片、Git 提交或远端地址。连接确实需要凭据时，只记录钥匙串、密码管理器或 secret manager 中的别名；发现疑似秘密时，校验失败并立即停止写入、提交和推送。
...
## 安全边界：网络事实可以同步，密钥绝不入库

本插件把网络事实与认证秘密严格分开。设备、服务、访问路径和已确认的排错结论可以进入私人状态；任何能够直接或间接完成认证的秘密都绝对禁止持久化。

以下内容不得写入私人状态、状态卡、过程卡、Git 提交、远端 URL 或命令示例：

- 密码、token、API key、私钥、认证密钥、cookie 或会话值；
- 恢复码、一次性验证码、登录二维码、订阅链接或含凭据的 URL；
- 完整配置导出、原始日志、命令流水或可能夹带秘密的证据包。

需要认证信息时，只能保存钥匙串别名、密码管理器条目名称、secret manager 引用或环境变量名称，不能保存实际值。GitHub 私人仓库只是网络状态的同步边界，不是秘密存储；仓库可见性为 private 也不会放宽这条规则。

每次写入、提交或推送前都要运行校验器并检查暂存差异。发现疑似秘密时，不复述其内容，立即停止后续操作；如果秘密已经进入仓库，应先轮换凭据，再把历史清理作为单独的明确授权任务处理。

校验器采用启发式扫描，可以拦截常见密钥和凭据形态，但不能构成数学意义上的零泄漏证明。人工检查暂存文件和 Git 历史仍是发布或同步前的最后一道边界。

主机名、地址、域名、用户名、拓扑、服务版本和验证时间仍属于敏感资料。项目卡片只提取完成收尾所需的最少内容，不复制完整私人网络清单。
