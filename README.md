...
还在不同笔记和聊天记录之间翻找，逐条记忆每台设备的 Tailscale IP、Nginx 反代、Docker 端口和连接关系？
...
从 Tailscale 设备地址，到 Nginx 反代、域名和 TLS，再到 Docker 容器及其端口、SSH、VPN 与跨设备访问路径，`network-state-closeout` 都能把这些零散信息整理成一份跨项目、跨设备通用的私人网络上下文，并在每次使用前确认它仍是最新状态。

具体来说：

- **插件保存稳定能力**：记录规范、同步门禁、隐私边界、校验脚本和收尾流程随插件分发。
- **私人目录保存动态事实**：真实设备、Nginx 反代、域名、TLS、端口、上游、访问路径和排错结论不进入插件。
- **GitHub 私人仓库负责跨设备同步**：启用后，查询和更新前必须先通过最新状态检查。
- **Codex 只读取当前任务需要的资料**：查询服务不必加载完整拓扑，排查路径也不必读取全部设备清单。
- **完成的动作沉淀为两类卡片**：状态卡说明现在能不能用，过程卡保留实现路线和未来排查入口；网络相关卡片还能作为结构化证据回写私人网络状态。
- **每次卡片回写都有私有回执**：共享交接编号与 `handoffs.md` 账本让重复处理不会重复写入，也能定位本次应用、阻塞或跳过的结果。
...
- `network-state`：维护独立私人目录中的设备、Nginx 与其他反向代理、域名、TLS、端口、上游服务、访问路径、拓扑和可复用排错事实。
- `action-closeout-cards`：在动作完成后分别生成状态卡与技术路线过程卡；网络相关卡片还会生成可校验的交接块。

组合工作流会让卡片和网络状态形成双向闭环。卡片保存某次动作的最终状态、技术路线与验证证据；`network-state` 核对其中声明的稳定候选事实，把确认结果写入唯一的当前网络状态文档，再将落库回执写回两张卡。完整私人网络资料不会被复制到项目卡片，卡片内容也不会被整段复制进私人状态。
...
日常网络工作
  ...
  -> 生成状态卡：当前事实候选与验证证据
  -> 生成过程卡：稳定路线与排错候选
  -> 校验共享交接编号、角色、隐私等级与回执字段
  -> network-state 核对并自主写入确认的稳定事实
  -> 在私有 handoffs.md 记录处理回执并校验私人资料
  -> 把应用、阻塞或跳过结果回填到两张卡
  -> 可选：登记到项目现有状态入口
```
...
└── skills/
    ...
    │   ├── SKILL.md
    │   ├── agents/openai.yaml
    │   ├── assets/profile-template/handoffs.md
    │   ├── references/card-ingestion.md
...
```text
处理这次网络变更并生成状态卡和技术路线过程卡；校验两张卡的网络状态交接块，自主把已确认的稳定候选事实落库，并把处理回执写回卡片。
```

从已有卡片维护网络状态：

```text
$network-state 读取这组状态卡和过程卡，校验共同的交接编号，核对其中已确认的稳定候选事实并自主落库；不要重复处理已经应用的交接。
```
...
- 私人仓库只保存状态 Markdown、精简的 `handoffs.md` 交接账本和 `.gitignore`，不保存插件代码、脚本、完整卡片、原始日志或配置导出。
...
| 私人网络状态 | 项目状态卡与过程卡 |
|---|---|
| 唯一当前事实源，保存结构化、可复用的设备与连接事实 | 同时面向人和 Codex 的单次动作证据包 |
| 保存在独立私人目录 | 保存在当前项目文档目录 |
| 可包含操作所必需的私人端点 | 默认只引用设备、服务和配置别名 |
| 不保存聊天过程或完整日志 | 不复制完整清单、拓扑或原始配置 |
| 通过 `handoffs.md` 防止重复落库并保留权威回执 | 两张卡共享交接编号，并接收落库回执 |
| 通过 `validate_profile.py` 校验 | 通过 `validate_card_pair.py`、内容分离检查和敏感信息扫描 |

状态卡只回答“现在是什么状态”，并声明可进入网络状态的当前事实候选；过程卡只回答“按什么技术路线实现、以后从哪里排查”，并声明可复用的路线和排错候选。两者都不是当前网络事实的最终权威。
...
```bash
python3 plugin/network-state-closeout/skills/action-closeout-cards/scripts/validate_card_pair.py \
  --status-card /path/to/status-card.md \
  --process-card /path/to/process-card.md
```
...
当前版本已经具备插件清单、初始化引导 skill、日常网络状态自主维护、状态卡与过程卡的机器可读交接、幂等回执账本、独立 GitHub 私人同步引导模块、强制最新检查、内容模板和本地校验能力。安装时接入用户选定的插件市场即可。
